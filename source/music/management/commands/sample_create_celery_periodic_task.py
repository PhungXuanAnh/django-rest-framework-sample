...
def create_interval_task():
    ...
    (
        interval_schedule_every_10_seconds,
        created,
    ) = IntervalSchedule.objects.get_or_create(
        every=10,
        period=IntervalSchedule.SECONDS,  # MICROSECONDS MINUTES HOURS DAYS
    )
...
def create_cron_task_run_at_timezone_Asia_Ho_Chi_Minh(asia_hcm_hour):
    schedule, _ = CrontabSchedule.objects.get_or_create(
        ...
        timezone=pytz.timezone(
            "Asia/Ho_Chi_Minh"
        ),  # celeryconfig: enable_utc = True (default), timezone = "utc" (default)
    )
